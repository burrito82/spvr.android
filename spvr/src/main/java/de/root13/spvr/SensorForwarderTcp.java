 ...
 */
public class SensorForwarderTcp implements SensorEventListener, Runnable {
    ...
    public SensorForwarderTcp(final Context context, final SensorManager sensorManager, final TextView txtLabel) {
        mContext = context;
    ...
    public void InitNetwork() {
        if (mSocket == null && mInetAddress != null) {
            try {
                //mInetAddress = InetAddress.getByName("192.168.192.57");
                mSocket = new Socket(mInetAddress, 4321);
                //mSocket.setSendBufferSize(4 * 5);
                mSocket.setReuseAddress(true);
                mSocket.setTcpNoDelay(true);
                //mSocket.setTrafficClass(...);
            } catch (IOException e) {
                Toast.makeText(mContext, "Could not initialize connection!", Toast.LENGTH_SHORT).show();
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                Toast.makeText(mContext, "SensorForwarderTcp.run() failed!", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            }
        }
    }
